...
const Remotes = ({product,addToCart}) => {
  ...
  useEffect(() => {
    ...
    return (<>
      ...
      {Object.keys(product).map((p)=>{return <Slide left key={p}><div style={sw?{height: "61vh",fontSize:"1.5vw", fontFamily: "'Roboto Slab', serif" }:{height: "65vh",fontSize:"1.5vh", backgroundColor: "#ebeaeb", fontFamily: "'Roboto Slab', serif" }} className="flex md:flex-row flex-col w-full hover:text-white rhover">
               <img  src={product[p].poster} style={{padding:"3vw 5vw"}}  alt='img'></img>
              <div style={sw?{padding:"1.56vw"}:{padding:"1.56vh 3vh"}} className='p-5 w-full space-y-8'>
