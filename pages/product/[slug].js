...
const Post = ({ buyNow, addToCart, product }) => {
  ...
  function selecteh(e) {
    setselectedexactheight(e.target.value)
  }
  function selectew(e) {
    setselectedexactwidth(e.target.value)
  }
  function selectw(e) {
  ...
  function selectcolor(e) {
    ...
    document.getElementById(e.target.id).classList.add("border-black")
      ...
      <div style={{width:"90vw"}} className=" md:py-20">
        ...
        <div className='w-full  flex-col md:flex-row flex   '>
          <div style={{width:"44vw"}} className=' '><img alt="ecommerce" className=" w-full p-16 md:p-0  " src={product.variants[selectedcolor].img} />
            ...
            <div className='w-full  grid grid-flow-col pr-10'>
              ...
              <div style={{fontFamily:"'lato',sans-serif"}} className='ml-8 '>
                ...
                <p className='leading-loose '>{selectedheight}{eval(selectedexactheight)} inches</p>
                <p className='leading-loose '>{selectedwidth}{eval(selectedexactwidth)} inches</p>
          ...
          <div style={{width:"55vw",backgroundColor:"#ebeaeb"}} className='flex flex-col  px-2 md:px-14 '>
            ...
            <div className="grid md:h-2/5 w-full px-8   h-80 mb-8 overflow-y-scroll bg-white  ">
            <p className='  font-medium pt-2'>Color Selection</p>
              {
               
              Object.keys(product.variants).map((p) => { return <div  key={p} ><div id={p} onClick={(e) => { selectcolor(e) }} style={{ backgroundColor: `${product.variants[p].colorcode}` }} className=" h-32 mt-8  cursor-pointer w-32"></div><span className='text-sm  '>{product.variants[p].color}</span></div> })}
            </div>
          ...
          {({ open }) => (
            <>
              <Disclosure.Button className="flex w-full justify-between items-center bg-white px-4 py-4 text-left text-sm font-medium text-purple-900  focus:outline-none focus-visible:ring focus-visible:ring-purple-500 focus-visible:ring-opacity-75">
...
{isenable && <div style={{height:"70vh",width:"60vw"}} className='fixed top-20 right-80 bg-white border-4 z-50'>
  <MdOutlineCancel onClick={measuringguide} className="text-xl float-right m-2" />
  </div>}
              </Disclosure.Panel>
            </>
          )}
        ...
        <Disclosure as="div" className="mt-2">
          {({ open }) => (
            <>
              <Disclosure.Button className="flex w-full justify-between  bg-white px-4 py-4 text-left text-sm font-medium text-purple-900 items-center focus:outline-none focus-visible:ring focus-visible:ring-purple-500 focus-visible:ring-opacity-75">
              ...
              <Disclosure.Panel className="px-4 pt-2 pb-8 text-sm  text-gray-500 flex justify-evenly bg-white">
               <button className='border-2  py-2 px-6'>Manual</button>
               <button className='border-2 py-2 px-6'>Motorized</button>
              </Disclosure.Panel>
            </>
          )}
        </Disclosure>
        ...
        <div className='grid grid-flow-row md:grid-flow-col mt-20'>
          ...
          <div>
            <div className='flex mt-12 mb-20 '>
              ...
              <button onClick={checkServiceablilty} className='ml-2 text-slate-700 hover:text-white btn border-0 py-2 px-4  focus:outline-none  '>Check</button>
            </div>
            </div>
