...
{!sw && <div onClick={toggleCart} onMouseEnter={() => setdropDown(false)} className="cart  absolute right-2 top-2   cursor-pointer">
            ...
            {dropDown && <div className='absolute right-5  z-40 top-10 md:top-20 border shadow-lg bg-white  px-4 py-3  acctext '>
                <ul>
                    <a className='hover:text-yellow-500  font-bold' href={"/myaccount"}><li className='account'>Account</li></a>
                    <a className='hover:text-yellow-500  font-bold' href={'/orders'}><li className='account'>Orders</li></a>
                    <li className='hover:text-yellow-500 account font-bold cursor-pointer' onClick={logout}>Logout</li>
                    {admin == "true" && <a className='hover:text-yellow-500  font-bold' href={"/adminpanel"}><li className='account'>Admin Panel</li></a>}
                </ul>
            </div>}
                    ...
                    Object.keys(data).map((p) => {
                        return (
                            <div key={p} className=" group flex ">
                                <a  id={data[p]} onMouseOver={() => setlistd(true)} onMouseEnter={function (e) { getData(e); }} className='border-l-2  px-4 py-3 text font-sans category hover:text-black font-semibold ' href={`${process.env.NEXT_PUBLIC_HOST}/category/${data[p]}`} >{data[p]}</a>
                                {subcategory && listd && <li className=' bg-slate-100  transform scale-0 group-hover:scale-100 absolute  top-12 z-40  text-gray-700  flex flex-col  '>{Object.keys(subcategory).map((s) => {
                                    return <a className='py-3 hover:border-l-4 border-black  px-6  hover:text-slate-900 hover:font-semibold   category '  href={`${process.env.NEXT_PUaC_HOST}/sub/${subcategory[s]}`} key={s}>{subcategory[s]}</a>
                                })}</li>}
                            </div>
                        )
